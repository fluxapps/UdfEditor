...
```php
...
use srag\Notifications4Plugin\UdfEditor\x\Utils\Notifications4PluginTrait;
...
class x {
...
...
<#x>
<?php
\srag\Notifications4Plugin\UdfEditor\x\Repository::getInstance()->installTables();
?>
```
 ...
 * ...
 *
 * @ilCtrl_isCalledBy srag\Notifications4Plugin\UdfEditor\x\Notification\NotificationsCtrl: x
 */
class x
{
    ...
...
```php
if (\srag\Notifications4Plugin\UdfEditor\x\Notification\Repository::getInstance()->migrateFromOldGlobalPlugin(x::TEMPLATE_NAME) === null) {

	$notification = \srag\Notifications4Plugin\UdfEditor\x\Notification\Repository::getInstance()->factory()->newInstance();

	$notification->setName(x::TEMPLATE_NAME);

	// TODO: Fill $notification with your default values

	\srag\Notifications4Plugin\UdfEditor\x\Notification\Repository::getInstance()->storeNotification($notification);
}
```
...
// Your class must implement the interface `srag\Notifications4Plugin\UdfEditor\x\Sender\Sender`
```
...
Your class must implement the interface `srag\Notifications4Plugin\UdfEditor\x\Parser\Parser`
